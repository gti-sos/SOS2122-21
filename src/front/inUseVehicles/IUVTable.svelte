<script>
	...
	async function getIuv(parametros="",busqueda=false) {
		...
		if(busqueda){
			...
			resBusqueda = await fetch("https://sos2122-21.herokuapp.com/api/v1/in-use-vehicles"+parametros);
		}
		else{
			res = await fetch("https://sos2122-21.herokuapp.com/api/v1/in-use-vehicles"+parametros+`?offset=${defaultOffset}&limit=${limitPag}`);
		}
	...
	async function getIuvPag(offset){
		...
		const res = await fetch(`https://sos2122-21.herokuapp.com/api/v1/in-use-vehicles?from=${yFrom}&to=${yTo}&offset=${off}&limit=${limitPag}`);
		if(res.ok){
			const data = await res.json();
				iuv = data;
				await delay(50);
	...
	async function loadInitialData() {
		...
		await fetch("https://sos2122-21.herokuapp.com/api/v1/in-use-vehicles/loadInitialData").then(async function (res) {
			getIuv();
			visibilidad = true;
			color="success";
	...
	async function deleteIuv(country,year){
		...
		const res = await fetch("https://sos2122-21.herokuapp.com/api/v1/in-use-vehicles/"+id, {
			method: "DELETE",
		}).then(async function (res) {
			getIuv();
			visibilidad = true;
